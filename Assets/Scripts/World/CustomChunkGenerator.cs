...
namespace Brickcraft.World.CustomGenerator
{
    public class CustomChunkGenerator
    {
            ...
            for (int bx = 0; bx < 16; bx++)
            {
                ...
                for (int bz = 0; bz < 16; bz++)
                {
                    ...
                    for (int by = 127; by >= 0; --by)
                    {
                        ...
                        if (c.GetBlockType(bx, by, bz) == BlockType.Stone)
            ...
            for (int bx = 0; bx < 16; ++bx)
            {
                ...
                for (int bz = 0; bz < 16; ++bz)
                {
                    ...
                    for (int by = 64; by < 128; ++by)
                    {
                        ...
                        if (c.GetBlockType(bx, by, bz) == BlockType.Grass && c.GetBlockType(bx, by + 1, bz) == (byte)BlockType.Air)
                        {
                            ...
                            if (by < 110 && bx % 4 == 0 && bz % 4 == 0)
                            {
                                ...
                                if (forestDens > 0.005)
                                {
                                    ...
                                    if (c.GetBlockType(randX, by, randZ) == BlockType.Grass)
                                        GenerateTree(c, randX, by, randZ);
                                    
                                    else if (biome == BIOME_TYPE.DESERT && c.GetBlockType(randX, by, randZ) == BlockType.Sand)
        ...
        {
            int by;
            for (by = y; c.GetBlockType(x,y,z) == 0 && by < 128; ++by) ;
